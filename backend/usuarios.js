...
router.get(
  ...
  async (req, res) => {
    ...
    const [rows] = await db.execute("SELECT id, username, nombre, apellido, activo FROM usuarios WHERE id=?", [id]);
...
router.post(
  ...
  async (req, res) => {
    ...
    const [result] = await db.execute("INSERT INTO usuarios (username, apellido, nombre, password_hash) VALUES (?,?,?,?)", [username, apellido, nombre, hashedPassword]);
...
router.delete("/:id", verificarAutenticacion, verificarAutorizacion("admin"), validarId, verificarValidaciones, async (req, res) => {
  const id = Number(req.params.id);
...
router.get("/:id/roles", verificarAutenticacion, validarId, verificarValidaciones, async (req, res) => {
  const id = Number(req.params.id);
...
router.post("/:id/roles", verificarAutenticacion, verificarAutorizacion("admin"), validarId, body("rolId").isInt({ min: 1 }), verificarValidaciones, async (req, res) => {
...
router.delete("/:id/roles/:rolId", verificarAutenticacion, verificarAutorizacion("admin"), validarId, param("rolId").isInt({ min: 1 }), verificarValidaciones, async (req, res) => {
  const usuarioId = Number(req.params.id);
