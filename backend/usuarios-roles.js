...
router.get("/usuarios/:usuarioId/roles/:rolId", validarUsuarioId, validarRolId, verificarValidaciones, getUsuariosRoles);

// GET /usuarios-roles/roles/2/usuarios/1
router.get("/roles/:rolId/usuarios/:usuarioId", validarRolId, validarUsuarioId, verificarValidaciones, getUsuariosRoles);
...
async function getUsuariosRoles(req, res) {
  ...
  if (rows.length === 0) {
    return res.status(404).json({ success: false, message: "Usuario/rol no encontrado" });
  }
